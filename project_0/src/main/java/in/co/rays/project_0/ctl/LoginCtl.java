...
import in.co.rays.project_0.form.ForgetPasswordForm;
import in.co.rays.project_0.form.LoginForm;
import in.co.rays.project_0.form.UserRegistrationForm;
...
public class LoginCtl extends BaseCtl {
	...
	public String Display(@ModelAttribute("form") LoginForm form, HttpSession session, Model model, Locale locale) {
		...
		if (session.getAttribute("user") != null) {
			session.invalidate();
			String msg = messageSource.getMessage("label.logoutsuccess", null, locale);
	...
	public String submit(Locale locale, @ModelAttribute("form") @Valid LoginForm form, BindingResult bindingResult,
		...
		if (OP_SIGNIN.equalsIgnoreCase(form.getOperation())) {
			...
			if (dto != null) {
				...
				System.out.println(form.getUri()+"   check null uri");
				if (form.getUri() != null  && form.getUri().length() > 0 && form.getUri().toString().trim() != "") {

					return "redirect:/" + form.getUri().replace("/project_0/", "");
